...
comments: post['comments'] is List<dynamic>
    ? (post['comments'] as List<dynamic>)
        .map<String>((e) {
            if (e is Map<String, dynamic> && e.containsKey('content')) {
                // Extract 'content' field and ensure it's a String
                return e['content'] is String ? e['content'] as String : e['content'].toString();
            }
            return ''; // Default to an empty string if 'content' is not available
        })
        .toList()
    : <String>[], // Explicitly provide an empty List<String> if not a List<dynamic>
...
void _handleLike(String postId, String userId, {bool isLiked = false}) async {
  // Toggle the isLiked status
  bool newIsLiked = !isLiked;

  try {
    // Call the provider's toggleLike method
    final result = await Provider.of<UserProvider>(context, listen: false).toggleLike(
      postId: postId,
      userId: userId,
      isLiked: newIsLiked,
    );
    ...
    if (result.containsKey('error')) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Failed to toggle like: ${result['error']}'),
        ),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Post ${newIsLiked ? 'liked' : 'liked'} successfully'),
        ),
      );
      // Optionally, refresh the posts or update the UI
      setState(() {
        // Fetch posts or update UI
        _forumPostsFuture = Provider.of<UserProvider>(context, listen: false).getAllForumPosts();
      });
    }
  } catch (e) {
...
void _showCommentsDialog(List<String> comments) {
  showDialog(
    context: context,
...
void _showAddCommentDialog(String postId) {
  ...
  // Access the UserProvider and retrieve userId
...
Future<void> _submitComment({
  ...
  try {
    final response = await http.post(
      Uri.parse('http://10.0.2.2:3000/api/posts/$postId/comments'),
      headers: {
    ...
    if (response.statusCode == 201) {
      ...
      _fetchForumPosts(); // Refresh the forum posts to show the new comment
    } else {
      print('Failed to submit comment. Status code: ${response.statusCode}');
    }
  } catch (e) {
    print('Error while submitting comment: $e');
  }
}
...
children: [
  _buildIconTextRow(
    ...
    text: (likes ?? 0).toString(),
    onTap: () => _handleLike(postId, currentUser??"", isLiked: true),
  ),
  SizedBox(width: 16),
  _buildIconTextRow(
    ...
    onTap: () => _showCommentsDialog(comments ?? []),
                ...
                Spacer(),
              _buildIconTextRow(
                iconPath: 'assets/images/add.png', // Add an icon for adding comments
                text: '',
                onTap: () => _showAddCommentDialog(postId),
              ),
                    SizedBox(width: 16),
